...
(function (window, document, Math) {
    ...
    function DatePickerScroll (el, options) {
...
// INSERT POINT: indicator._move
    ...
    DatePicker.prototype = {
        ...
        scrollInit: function (index, num) {
            ...
            //iscroll 初始化
            self.scrollArray[index] = new DatePickerScroll(id, {
                probeType: 3,
                scrollX: false,
                scrollY: true,
                useTransition: true,
        ...
        //滚动距离计算
        scrollSite: function (type, itemHeight) {
            var self = this;
            var myScroll = self.scrollArray[type];
            ...
            myScroll.on('scroll', function () {
                ...
                // var moveingY = parseInt(this.y);
                // var index = parseInt(Math.abs(moveingY) / itemHeight);//item index
                // var h = Math.abs(moveingY % itemHeight);//滚动多余不足一行的高度
                ...
                var wd_height = document.body.scrollHeight - 1;

                if ( this.pointY < 1 ) {
                    var count = this.scroller.childElementCount;
                    var nowScroll = this.scroller.childNodes[count - 3];//当前项节点
                    self.SetItemList(type, nowScroll);
                    self.isScrollTo = true;
                    this.scrollTo(0, this.maxScrollY, 0);
                } else if ( this.pointY > wd_height ) {
                    var nowScroll = this.scroller.childNodes[2];//当前项节点
                    self.SetItemList(type, nowScroll);
                    self.isScrollTo = true;
                    this.scrollTo(0, 0, 0);
                }
            ...
            myScroll.on('scrollEnd', function () {
                ...
                var index = parseInt(Math.abs(moveY) / itemHeight) + 2;//item index
                var nowScroll = this.scroller.childNodes[index];//当前项节点
                self.SetItemList(type, nowScroll);
                ...
                if( self.options.type == 3 ){
                    ...
                    if( nowPlace > self.monthLen ){
                        var hiddenLen = nowPlace - self.monthLen;
                        var moveLen = self.scrollArray[2].y + hiddenLen * itemHeight;
                        self.textArray[2].value = self.monthLen;
                        self.scrollArray[2].scrollTo(0, moveLen, 0);
                    }
                }
            });
        },
